...
**Reconsider:** Good pattern, but Redis adds deployment complexity. Evaluate if needed
...
`old/backend/home-observer/units_of_measurement.json` Complete SI/US unit mapping for
OpenHAB QuantityTypes.

### 3. Docker Compose Structure

`old/docker-compose.yaml` Clean service separation, environment handling.
