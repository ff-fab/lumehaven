...
This document describes the versioning strategy for the **lumehaven project**, a monorepo containing multiple packages (backend, frontend, etc.). All packages share a single version number to maintain consistency across the project.
...
The `.dev` suffix indicates a development version between official releases and includes git commit hash and date.
...
✅ **Single version for entire project** - All packages release together
✅ **Zero manual version bumps** - Version automatically derives from git tags
✅ **CI/CD friendly** - Deploy from tags, version is deterministic
✅ **Git integration** - Version always matches deployed code
✅ **Development clarity** - `.dev` suffix shows code is between releases
✅ **Monorepo scalability** - Adding packages doesn't complicate versioning
